...
class _CheckoutScreenState extends State<CheckoutScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    bool kmWiseCharge = CheckOutHelper.isKmWiseCharge(configModel: configModel);
        ...
        Expanded(child: CustomScrollView(slivers: [
          ...
          SliverToBoxAdapter(child: Consumer<LocationProvider>(builder: (context, locationProvider, _) {
            return Consumer<CheckoutProvider>(builder: (context, checkoutProvider, _) {
              ...
              if(checkoutProvider.orderType == OrderType.delivery && kmWiseCharge) {
                deliveryCharge = checkoutProvider.distance * configModel.deliveryManagement!.shippingPerKm!;
                ...
                Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Expanded(flex: 6, child: Container(
                    ...
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      ...
                      PaymentDetailsWidget(total: (widget.amount ?? 0) + (deliveryCharge ?? 0)),
                      const SizedBox(height: Dimensions.paddingSizeDefault),

                      if(!ResponsiveHelper.isDesktop(context)) PartialPayWidget(totalPrice: (widget.amount ?? 0) + (deliveryCharge ?? 0)),
                  ...
                  if(isDesktop) Expanded(flex: 4, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    PartialPayWidget(totalPrice: (widget.amount ?? 0) + (deliveryCharge ?? 0)),
        ...
        if(!isDesktop) Consumer<CheckoutProvider>(
          builder: (context, checkoutProvider, _) {
            double? deliveryCharge =  CheckOutHelper.getDeliveryCharge(
              ...
              minShippingCharge: configModel.deliveryManagement?.minShippingCharge ?? 0,
              ...
              child: Column(children: [
                ...
                UpsideExpansionWidget(
                  title: ItemViewWidget(
                    ...
                    subTitle: PriceConverterHelper.convertPrice(widget.amount! + deliveryCharge),
                    titleStyle: rubikMedium.copyWith(fontSize: Dimensions.fontSizeExtraLarge, color: Theme.of(context).primaryColor),
                  ),
                  children: [
  ...
  Future<void> _onInitLoad() async {
    ...
    if(_isLoggedIn || (splashProvider.configModel?.isGuestCheckout ?? false)) {
      ...
      CheckOutHelper.selectDeliveryAddressAuto(orderType: checkoutProvider.orderType, isLoggedIn: (_isLoggedIn || isGuestCheckout), lastAddress: addressModel);
