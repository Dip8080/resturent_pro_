...
class HomeScreen extends StatefulWidget {
  ...
  static Future<void> loadData(bool reload, {bool isFcmUpdate = false}) async {
    ...
    await wishListProvider.initWishList();

    await productProvider.getLatestProductList(1, reload);
    if(reload || productProvider.popularLocalProductModel == null){
      await productProvider.getPopularLocalProductList(1,  true, isUpdate: false);
    }
    ...
    await productProvider.getFlavorfulMenuProductMenuList(1, reload);
    await productProvider.getRecommendedProductList(1, reload);
    await bannerProvider.getBannerList(reload);
    await searchProvider.getCuisineList(isReload: reload);
    await searchProvider.getProductSearchTagList(isReload: reload);
    await searchProvider.getPoplarTagList(isReload: reload);
    await frequentlyBoughtProvider.getFrequentlyBoughtProduct(1, reload);
      ...
      child: Scaffold(
        ...
        body: RefreshIndicator(
          ...
          child: CustomScrollView(controller: _scrollController, slivers: [
                    ...
                    child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, crossAxisAlignment: CrossAxisAlignment.start,   children: [
                      if(scrollingRate < 0.01)
                        Column(crossAxisAlignment: CrossAxisAlignment.start,  children: [
                          ...
                          Text(getTranslated('current_location', context)!, style: rubikMedium.copyWith(
            ...
            SliverToBoxAdapter(child: Consumer<ProductProvider>(
              builder: (context, productProvider, _ ) {
                return (productProvider.latestProductModel?.products?.isEmpty ?? false) ? const SizedBox()
                    : Center(child: SizedBox(width: Dimensions.webScreenWidth, child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [

                    Padding(
                      padding: EdgeInsets.symmetric(horizontal: isDesktop ? 0 : Dimensions.paddingSizeDefault, vertical: Dimensions.paddingSizeDefault),
                      child: ProductViewWidget(productType: ProductType.latestProduct, scrollController: _scrollController),
                    ),
                  ],
                )));
              }
