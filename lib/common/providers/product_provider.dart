...
class ProductProvider extends ChangeNotifier {
  ...
  Future<void> getLatestProductList(int offset, bool reload, { bool isUpdate = true}) async {
    ...
    if(_latestProductModel == null || offset != 1) {
      ApiResponseModel? response = await productRepo?.getLatestProductList(offset, productSortProvider.selectedShotType);
  ...
  Future<void> getPopularLocalProductList(int offset, bool reload, { bool isUpdate = true}) async {
    ...
    ApiResponseModel? response = await productRepo?.getPopularProductList(offset);
  ...
  Future<void> getRecommendedProductList(int offset, bool reload, { bool isUpdate = true}) async {
    ...
    if((_recommendedProductModel == null) || (offset != 1)) {
      ApiResponseModel? response = await productRepo?.getRecommendedProductApi(offset);
      if (response?.response?.data != null && response?.response?.statusCode == 200) {
        if(offset == 1){
  ...
  Future<void> getFlavorfulMenuProductMenuList(int offset, bool reload, { bool isUpdate = true}) async {
    ...
    if(_flavorfulMenuProductMenu == null || offset != 1) {
      ApiResponseModel? response = await productRepo?.getFlavorFulMenuProductApi(offset);
      if (response?.response?.data != null && response?.response?.statusCode == 200) {
        if(offset == 1){
