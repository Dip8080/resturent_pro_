...
class CheckOutHelper{
  ...
  static AddressModel? getDeliveryAddress({
    ...
    if(deliveryAddress != null && !isAddressInCoverage(branchProvider.getBranch(),  deliveryAddress)) {
  ...
  static bool isKmWiseCharge({required ConfigModel? configModel}) => configModel?.deliveryManagement?.status ?? false;
    ...
    if(isAvailable) {
      ...
      if(CheckOutHelper.isKmWiseCharge(configModel: configModel)) {
        ...
        bool isSuccess = await checkoutProvider.getDistanceInMeter(
          LatLng(
            double.tryParse('${configModel?.branches?[checkoutProvider.branchIndex]?.latitude}') ?? 0,
            double.tryParse('${configModel?.branches?[checkoutProvider.branchIndex]?.longitude}') ?? 0,
          ),
          LatLng(
